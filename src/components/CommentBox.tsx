...
const CommentBox = ({
    ...
    const fetchComments = async () => {
      try {
        const response = await fetch(
          `/api/comments/${postId}/get-post-comments`,
        );
  ...
  const submitReply = async (e: React.FormEvent, commentId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Refetch comments to get new reply
        const response = await fetch(
          `/api/comments/${postId}/get-post-comments`,
        );
        const data = await response.json();
        ...
        <div className="_comment_area">
          <div className="_comment_details">
            ...
            <div className="_total_reactions">
              ...
              <span className="_total">{comment.reactions.length}</span>
            </div>
  ...
  // Number of previous comments hidden
  const hiddenCount = comments.length > 0 ? comments.length - 1 : 0;
  ...
  return (
    <div>
      {loading ? (
        ...
        <>
          {/* Show button only if there are previous comments hidden */}
          ...
          {comments.length > 0 && renderComment(comments[comments.length - 1])}

          {/* Show previous comments below latest comment when toggled */}
          {showPrevious &&
            comments
              .slice(0, comments.length - 1) // all except latest
